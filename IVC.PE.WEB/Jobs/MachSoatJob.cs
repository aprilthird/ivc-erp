...
{
    public class MachSoatJob : CronJobService
    {
        public MachSoatJob(IScheduleConfig<MachSoatJob> config, ILogger<MachSoatJob> logger, IServiceScopeFactory scopeFactory, IEmailSender emailSender)
      : base(config.CronExpression, config.TimeZoneInfo, logger, scopeFactory, emailSender)
        ...
        {
            _logger.LogInformation("CronJob MachSoat starts.");
        ...
        public override async Task DoWork(CancellationToken cancellationToken)
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                var equips = await _context.Set<UspMachSoatFolding>().FromSqlRaw("execute EquipmentMachinery_uspMachSoatFolding")
                .IgnoreQueryFilters()
                .ToListAsync();
                ...
                var equipsResponsibles = await _context.EquipmentMachSOATFoldingApplicationUsers.ToListAsync();
                ...
                {
                    var responsibles = equipsResponsibles.FirstOrDefault(x => x.EquipmentMachSOATFoldingId == equip.FoldingId).UserId;

                    SendAlertMail(responsibles.Split(','), equip, users);

                    var renewal = await _context.EquipmentMachSOATFoldings.FirstOrDefaultAsync(x => x.Id == equip.FoldingId);
                    renewal.Days30 = true;
        ...
        private async void SendAlertMail(string[] responsibles, UspMachSoatFolding equip, List<ApplicationUser> users)
        {
            ...
            if (equip.SOATFileUrl != null)
            {
                ...
                attachment = new Attachment(ms, $"SOAT_{equip.SoatOrder}_{equip.Model}_{equip.Brand}_{equip.SerieNumber}.pdf");
            }
            ...
            foreach (var responsible in responsibles)
            {
                ...
                mailMessage.Subject = $"IVC - Vencimiento de SOAT - Equipos Maquinaria";
                ...
                if (user != null)
                {
                    ...
                    mailMessage.Body =
                        ...
                        $"Favor de coordinar la renovación del SOAT del equipo {equip.Model} - {equip.Brand} - {equip.SerieNumber} - que le pertenece a {equip.Tradename} y tiene como fecha de vencimiento  { equip.EndDateSOATString } en el centro de costo {equip.Center}. <br /><br />" +
                        $"Saludos <br />" +
                        $"Sistema IVC <br />" +
                        $"Control de Equipo";
