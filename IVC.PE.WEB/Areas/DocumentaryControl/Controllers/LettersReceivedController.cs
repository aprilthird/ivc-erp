...
namespace IVC.PE.WEB.Areas.DocumentaryControl.Controllers
{
    ...
    [Route("control-documentario/cartas-recibidas")]
    public class LettersReceivedController : BaseController
    {
        private readonly IOptions<CloudStorageCredentials> _storageCredentials;

        public LettersReceivedController(IvcDbContext context,
            ILogger<LettersReceivedController> logger,
        ...
        [HttpGet("emisores")]
        public async Task<IActionResult> GetSenders(int? page = null, int rowsPerPage = 5)
        {
            ...
            var model = await query
                .Select(x => new IssuerTargetSummaryViewModel
                {
                    ...
                    Acronym = x.Acronym,
                    Name = x.Name,
                    TotalCount = x.Letters.Where(x => x.ProjectId == projectId).Count()
                })
        ...
        {
            SqlParameter letterTypeParam = new SqlParameter("@LetterType", ConstantHelpers.Letter.Type.RECEIVED);
            ...
            //if (issuerTargetId.HasValue)
            //    query = query.Where(x => x.IssuerId == issuerTargetId.Value);
            ...
            //if (hasAnswer.HasValue)
            //    query = query.Where(x => x.LetterStatus.Any(s => s.Status == ConstantHelpers.Letter.Status.QUERY_US) && hasAnswer.Value == x.ReferencedBy.Any(r => r.Letter.LetterStatus.Any(s => s.Status == ConstantHelpers.Letter.Status.ANSWER)));
            ...
            //        ResponseTermDays = x.LetterStatus.Any(l => l.Status == ConstantHelpers.Letter.Status.QUERY_US || l.Status == ConstantHelpers.Letter.Status.OBSERVATION)
            //            ? x.ReferencedBy.Any(r => r.Letter.Type == ConstantHelpers.Letter.Type.SENT && r.Letter.LetterStatus.Any(l => l.Status == ConstantHelpers.Letter.Status.ANSWER))
            //            ? x.ReferencedBy.Where(r => r.Letter.Type == ConstantHelpers.Letter.Type.SENT && r.Letter.LetterStatus.Any(l => l.Status == ConstantHelpers.Letter.Status.ANSWER))
            //                .Select(r => (int?)EF.Functions.DateDiffDay(x.Date, r.Letter.Date)).FirstOrDefault()
            //                : EF.Functions.DateDiffDay(x.Date, DateTime.UtcNow) : null,
            //        Issuer = x.IssuerId.HasValue ? new IssuerTargetViewModel
            //        {
            //            Name = x.Issuer.Name
            //        } : null,
        ...
        public async Task<IActionResult> Create(LetterViewModel model)
        {
            ...
            {
                Type = ConstantHelpers.Letter.Type.RECEIVED,
                Code = model.Code,
                Name = model.Name,
                ProjectId = GetProjectId(),
                Subject = model.Subject,
                Date = model.Date.ToDateTime(),
                //ResponseTermDays = model.ResponseTermDays,
            ...
            if (model.File != null)
            {
                ...
                letter.FileUrl = await storage.UploadFile(model.File.OpenReadStream(),
                    ConstantHelpers.Storage.Containers.DOCUMENTARY_CONTROL, System.IO.Path.GetExtension(model.File.FileName), ConstantHelpers.Storage.Blobs.LETTERS_RECEIVED, letter.Code);
            }
            ...
            //}
            //await SendConfirmationEmail(model.File, employee, letter);
        ...
        public async Task<IActionResult> Edit(Guid id, LetterViewModel model)
        {
            ...
            //letter.ResponseTermDays = model.ResponseTermDays;
            letter.EmployeeId = model.EmployeeId;
            letter.IssuerId = model.IssuerId;
            ...
            if (model.File != null)
            {
                ...
                if (letter.FileUrl != null)
                    await storage.TryDelete($"{ConstantHelpers.Storage.Blobs.LETTERS_RECEIVED}/{letter.FileUrl.AbsolutePath.Split('/').Last()}",
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            var letter = await _context.Letters
                ...
                .FirstOrDefaultAsync(x => x.Id == id);
            if (letter == null)
                return BadRequest($"Carta Recibida con Id '{id}' no encontrada.");
            if (letter.LetterInterestGroups != null)
                _context.LetterInterestGroups.RemoveRange(letter.LetterInterestGroups);
            if (letter.LetterStatus != null)
                _context.LetterStatus.RemoveRange(letter.LetterStatus);
        ...
        //private async Task SendConfirmationEmail(IFormFile file, ApplicationUser user, Letter letter)
        {
            ...
            mailMessage.Subject = $"Documento recibido ({letter.Name})";
            ...
            mailMessage.Body =
                $"Hola {user.RawFullName},<br/>Favor de preparar y enviar el borrador de respuesta a la carta recibida adjunta al correo, el plazo máximo de respuesta es de {letter.ResponseTermDays} días calendarios.<br/><br/>Saludos<br/>Sistema IVC<br/>Control Documentario";
        ...
        public async Task<IActionResult> ImportData(IFormFile file)
        {
            using (var mem = new MemoryStream())
            {
                ...
                {
                    var workSheet = workBook.Worksheets.FirstOrDefault(x => x.Name == "ENVIADAS");
                    var counter = 2;
                    ...
                    {
                        var letter = new Letter();
                        letter.Type = ConstantHelpers.Letter.Type.SENT;
                        ...
                        var receiverString = workSheet.Cell($"C{counter}").GetString();
                        if (!string.IsNullOrEmpty(receiverString))
                        {
                            letter.LetterIssuerTargets = new List<LetterIssuerTarget>();
                            foreach (var receiver in receiverString.Split("; "))
                            {
                    ...
                    foreach (var entry in entries)
                    {
                        ...
                        if (letter != null && letter.FileUrl == null)
                        {
                            letter.FileUrl = await storage.UploadFile(entry.Open(),
                                ConstantHelpers.Storage.Containers.DOCUMENTARY_CONTROL, System.IO.Path.GetExtension(entry.Name), ConstantHelpers.Storage.Blobs.LETTERS_RECEIVED, letter.Code);
                            await _context.SaveChangesAsync();
                        }
                    }
                }
        ...
        public async Task<IActionResult> ExcelReport()
        {
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                var ws = wb.Worksheets.Add("Cartas Recibidas");
                ...
                using (MemoryStream stream = new MemoryStream())
                {
                    ...
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte de Cartas Recibidas.xlsx");
                }
        ...
        [HttpGet("enviar-correo/{id}")]
        public async Task<IActionResult> UpdateStatus(Guid id)
        {
            var request = await _context.Letters.FirstOrDefaultAsync(x => x.Id == id);

            var logAuths = await _context.LetterResponsibles
            ...
            foreach (var auth in logAuths)
            {
                ...
                mailMessage.Body =
                   ...
                   $"Se le informa con urgencia la entrega de la carta {request.Name} con el asunto {request.Subject}, la cual necesita una respuesta.<br />" +
                   $"<br />" +
