var Formula = function () {
    ...
    var options = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/finanzas/tipofianzas/listar"),
            dataSrc: "",
            data: function (d) {
                delete d.columns;
            }
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return new Promise((resolve) => {
                                $.ajax({
                                    url: _app.parseUrl(`/finanzas/tipofianzas/eliminar/${id}`),
                                    type: "delete",
                                    success: function (result) {
                                        ...
                                        swal.fire({
                                            type: "success",
                                            title: "Completado",
                                            text: "La formula ha sido eliminada con éxito",
    ...
    var form = {
        load: {
            edit: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/finanzas/tipofianzas/${id}`)
                }).done(function (result) {
                    let formElements = $("#edit_form");
                    formElements.find("[name='Id']").val(result.id);
                    formElements.find("[name='Name']").val(result.name);
                    formElements.find("[name='Code']").val(result.code);
                    formElements.find("[name='IsDirectCost']").prop("checked", result.isDirectCost);
        ...
        submit: {
            add: function (formElement) {
                ...
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl("/finanzas/tipofianzas/crear"),
                    type: "post",
                    data: data
                }).always(function () {
            ...
            edit: function (formElement) {
                ...
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl(`/finanzas/tipofianzas/editar/${id}`),
                    type: "put",
                    data: data
                }).always(function () {
                    $(formElement).find("input").prop("disabled", false);
