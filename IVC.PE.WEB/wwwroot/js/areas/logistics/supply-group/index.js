var SupplyGroup = function () {
    ...
    var options = {
        ...
        ajax: {
            ...
            dataSrc: "",
            data: function (d) {
                d.supplyFamilyId = $("#budget_supplyfamily_filter").val();
                delete d.columns;
            }
        },
        columns: [
    ...
    var form = {
        ...
        reset: {
            add: function () {
                addForm.resetForm();
                $(".select2-supply-family").prop('selectedIndex', 0).trigger("change");
        ...
        supplies: {
            init: function () {
                $.ajax({
                    ...
                    .done(function (result) {
                        $(".select2-supply-family").select2({
                            data: result
                        });
                        $(".select2-supplyfamily_filter").select2({
                            data: result
                        });
                    });
            },
            filter: function () {
                $("#budget_supplyfamily_filter").on("change", function () {
                    datatable.reload();
                });
            }
        }
    ...
    var events = {
        init: function () {
            ...
            $("#genEsxelSample").on("click", function () {
                window.location = _app.parseUrl(`/logistica/grupos/excel-modelo`);
            });
        }
    };
