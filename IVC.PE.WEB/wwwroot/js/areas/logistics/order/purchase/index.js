var Order = function () {
    ...
    var itemOpts = {
        ...
        columns: [
            ...
            },
            {
                ...
                data: "glosa",
                render: function (data, type, row) {
                    var tmp = `<textarea id="${row.id}_${num}_glosa" data-id="${row.id}" class="form-control" type="text" style="font-size:13px;">`;
                    tmp += `</textarea>`;
                    return tmp;
                }
            },
            ...
            },
            {
                ...
                width: "15%",
                render: function (data, type, row) {
                    var tmp = `<input id="${row.id}_${num}" data-id="${row.id}" class="form-control" type="number" value="${data}">`;
                    tmp += `</input>`;
                    return tmp;
                }
            },
            {
                ...
                data: "observations",
                render: function (data, type, row) {
                    var tmp = `<input id="${row.id}_${num}_price" data-id="${row.id}" class="form-control" type="number" >`;
                    tmp += `</input>`;
                    tmp += `<input id="${row.id}_${num}_obs" data-id="${row.id}" class="form-control" value="${data}" hidden>`;
                    tmp += `</input>`;
                    num++;
                    return tmp;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    tmp += `<input id="${row.id}_obs" data-id="${row.id}" class="form-control" value="${row.requestObservation}" hidden>`;
                    tmp += `</input>`;
                    return tmp;
                }
            },
            {
                title: "Precio Unitario c/ Dsctos.",
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    var tmp = "";
                    tmp += `<button data-id="${row.id}" class="btn btn-primary btn-sm btn-icon btn-dis" title="Emitir">`;
                    tmp += `<i class="la la-clipboard"></i></button> `;
                    return tmp;
                }
            }
        ],
        ...
        initEvents: function () {
            ...
            mainDatatable.on("click",
                ...
                function () {
                    ...
                    Swal.fire({
                        title: "¿Está seguro?",
                        text: "La Orden de Compra será emitido",
                        type: "warning",
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        ...
                                        swal.fire({
                                            type: "success",
                                            title: "Completado",
                                            text: "La Orden de Compra ha sido emitida con éxito",
                                    ...
                                    error: function (errormessage) {
                                        swal.fire({
                                            ...
                                            text: "Ocurrió un error al intentar emitir la Orden de Compra"
                                        });
                                    }
                                });
                            });
            ...
            mainDatatable.on("click",
                ...
                function () {
                    ...
                    Swal.fire({
                        title: "¿Está seguro?",
                        text: "La órden de compra será eliminada permanentemente",
                        type: "warning",
                        showCancelButton: true,
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        ...
                                        swal.fire({
                                            type: "success",
                                            title: "Completado",
                                            text: "La órden de compra ha sido eliminada con éxito",
                                            confirmButtonText: "Excelente"
                                        });
                                    },
                                    error: function (errormessage) {
                                        swal.fire({
                                            ...
                                            customClass: 'animated tada',
                                            confirmButtonText: "Entendido",
                                            text: "Ocurrió un error al intentar eliminar la órden de compra"
                                        });
                                    }
                                });
                            });
            ...
            itemEditDatatable.on("click",
                ".btn-dis",
                function () {
                    let $btn = $(this);
                    let id = $btn.data("id");
                    inDiscount = true;
                    form.load.discount(id);
                });
    ...
    var form = {
        load: {
            ...
            discount: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/logistica/ordenes/detalles/${id}`)
                })
                    .done(function (result) {
                        let formElements = $("#dis_form");
                        $("#edit_modal").modal("hide");

                        formElements.find("[name='Id']").val(result.id);
                        formElements.find("[name='FinancialDiscount']").val(result.financialDiscount);
                        formElements.find("[name='ItemDiscount']").val(result.itemDiscount);
                        formElements.find("[name='AdditionalDiscount']").val(result.additionalDiscount);
                        formElements.find("[name='IGV']").val(result.igv);
                        formElements.find("[name='ISC']").val(result.isc);
        ...
        submit: {
            add: function (formElement) {
                ...
                $.ajax({
                    url: _app.parseUrl("/logistica/ordenes/compra/crear"),
                    method: "post",
                    contentType: false,
                    processData: false,
                    data: data,
            ...
            import: function (formElement) {
                ...
                $.ajax({
                    url: "/logistica/ordenes/compra/importar",
                    type: "POST",
                    contentType: false,
                    processData: false,
                    data: data,
            ...
            discount: function (formElement) {
                ...
                let id = $(formElement).find("input[name='Id']").val();
                $.ajax({
                    url: _app.parseUrl(`/logistica/ordenes/descuentos/${id}`),
                    method: "put",
                    contentType: false,
                    processData: false,
                    data: data
                    ...
                    .done(function (result) {
                        ...
                        datatable.itemEditReload();
                        _app.show.notification.dis.success();
                    })
                    .fail(function (error) {
                        if (error.responseText) {
                            $("#dis_alert_text").html(error.responseText);
                            $("#dis_alert").removeClass("d-none").addClass("show");
                        }
                        _app.show.notification.edit.error();
                    });
            }
        ...
        reset: {
            ...
            discounts: function () {
                ...
                $("#dis_form").trigger("reset");
                $("#dis_alert").removeClass("show").addClass("d-none");
                inDiscount = true;
                $("#edit_modal").modal("show");
            }
        }
    ...
    var validate = {
        init: function () {
            ...
            discountForm = $("#dis_form").validate({
                submitHandler: function (formElement, e) {
                    e.preventDefault();
                    form.submit.discount(formElement);
                }
            });
        }
    };
    ...
    var modals = {
        init: function () {
            ...
            $("#dis_modal").on("hidden.bs.modal",
                function () {
                    form.reset.discounts();
                });
        }
    };
    ...
    var events = {
        init: function () {
            ...
            $("#g_descuentos").on("click", function () {
                load.submit.discount($("#dis_form"));
            });
        }
    };
