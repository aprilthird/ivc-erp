var Formula = function () {
    ...
    var options = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/control-documentario/responsables-de-permisos/listar"),
            dataSrc: ""
        },
        columns: [
            {
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return new Promise((resolve) => {
                                $.ajax({
                                    url: _app.parseUrl(`/control-documentario/responsables-de-permisos/eliminar/${id}`),
                                    type: "delete",
    ...
    var form = {
        load: {
            edit: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/control-documentario/responsables-de-permisos/${id}`)
        ...
        submit: {
            add: function (formElement) {
                ...
                $(formElement).find("[name='Responsibles']").val($(formElement).find("[name='select_users']").val().toString());
                let data = $(formElement).serialize();
                let $btn = $(formElement).find("button[type='submit']");
                $(formElement).find("input").prop("disabled", true);
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl("/control-documentario/responsables-de-permisos/crear"),
                    type: "post",
                    data: data
                }).always(function () {
            ...
            edit: function (formElement) {
                ...
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl(`/control-documentario/responsables-de-permisos/editar`),
                    type: "put",
                    data: data
                }).always(function () {
                    $(formElement).find("input").prop("disabled", false);
