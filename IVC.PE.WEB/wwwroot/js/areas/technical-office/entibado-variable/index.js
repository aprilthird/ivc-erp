var EntibadoVariable = function () {
    ...
    var options = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/oficina-tecnica/variables-entibado/listar"),
            dataSrc: "",
            data: function (d) {
                delete d.columns;
            }
        ...
        columns: [
            ...
            {
                //title: "Prof. Libre max. fondo Zanja (m)",
                data: "freeDitchFondo"
            },
            {
                //title: "Prof. Max. Zanja (m)",
                data: "maxDitch"
            },
            {
                //title: "Factor Uso",
            ...
            {
                //title: "Rendimiento Replanteado",
                data: "restatedPerformance"
            },
            {
                title: "Opciones",
                data: null,
        ...
        "columnDefs": [
            { className: "dt-body-right", "targets": [2, 5, 6, 7, 8, 9, 10, 11, 12 ,13] }
        ]
    };

    var datatable = {
        init: function () {
            mainDatatable = $("#entibado_datatable").DataTable(options);
            this.initEvents();
        },
        reload: function () {
        ...
        initEvents: function () {
            ...
            mainDatatable.on("click",
                ...
                function () {
                    ...
                    Swal.fire({
                        title: "¿Está seguro?",
                        text: "La variable entibado será eliminado permanentemente",
                        type: "warning",
                        ...
                        preConfirm: () => {
                            return new Promise((resolve) => {
                                $.ajax({
                                    url: _app.parseUrl(`/oficina-tecnica/variables-entibado/eliminar/${id}`),
                                    type: "delete",
                                    success: function (result) {
                                        ...
                                        swal.fire({
                                            type: "success",
                                            title: "Completado",
                                            text: "La variable de entibado ha sido eliminada con éxito",
                                    ...
                                    error: function (errormessage) {
                                        swal.fire({
                                            ...
                                            text: "Ocurrió un error al intentar eliminar la variable de entibado"
                                        });
                                    }
                                });
                            });
    ...
    var form = {
        load: {
            edit: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/oficina-tecnica/variables-entibado/${id}`)
                }).done(function (result) {
                    let formElements = $("#edit_form");
                    formElements.find("[name='Id']").val(result.id);
                    formElements.find("[name='Name']").val(result.name);
                    formElements.find("[name='Dimensions']").val(result.dimensions);
                    formElements.find("[name='LDimension']").val(result.lDimension);
                    formElements.find("[name='HDimension']").val(result.hDimension);
                    formElements.find("[name='Thickness']").val(result.thickness);
                    formElements.find("[name='Weight']").val(result.weight);
                    formElements.find("[name='FreeDitchTope']").val(result.freeDitchTope);
                    formElements.find("[name='FreeDitchFondo']").val(result.freeDitchFondo);
                    formElements.find("[name='MaxDitch']").val(result.maxDitch);
                    formElements.find("[name='UnitPrice']").val(result.unitPrice);
                    formElements.find("[name='UseFactor']").val(result.useFactor);
                    formElements.find("[name='Quantity']").val(result.quantity);
                    formElements.find("[name='restatedPerformance']").val(result.restatedPerformance);
        ...
        submit: {
            add: function (formElement) {
                ...
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl("/oficina-tecnica/variables-entibado/crear"),
                    type: "post",
                    data: data
                }).always(function () {
            ...
            edit: function (formElement) {
                ...
                $btn.addLoader();
                $.ajax({
                    url: _app.parseUrl(`/oficina-tecnica/variables-entibado/editar/${id}`),
                    type: "put",
                    data: data
                }).always(function () {
                    $(formElement).find("input").prop("disabled", false);
    ...
    var events = {
        refresh: function () {
            $("#btn-refresh").on("click", function () {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl("/oficina-tecnica/variables-entibado/actualizar"),
                    method: "put",
                    contentType: false,
                    processData: false
                })
