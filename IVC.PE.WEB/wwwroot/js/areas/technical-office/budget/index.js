var Budget = function () {
    ...
    var budgetDtOpt = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/oficina-tecnica/presupuestos/listar"),
            data: function (d) {
        ...
        columns: [
            ...
            },
            {
                ...
                width: "5%",
                data: function (result) {
                    if (result.contractualMO == "0.00")
                        return "";
                    else
                        return result.contractualMO;
                }
            },
            {
                //tile: "Contractual EQ",
                width: "5%",
                data: function (result) {
                    if (result.contractualEQ == "0.00")
                        return "";
                    else
                        return result.contractualEQ;
                }
            },
            {
                //tile: "Contractual SubContrato",
                width: "5%",
                data: function (result) {
                    if (result.contractualSubcontract == "0.00")
                        return "";
                    else
                        return result.contractualSubcontract;
                }
            },
            {
                //tile: "Contractual Materiales",
                width: "5%",
                data: function (result) {
                    if (result.contractualMaterials == "0.00")
                        return "";
                    else
                        return result.contractualMaterials;
                }
            },
            {
            ...
            {
                //tile: "Collaborator MO",
                width: "5%",
                data: function (result) {
                    if (result.collaboratorMO == "0.00")
                        return "";
                    else
                        return result.collaboratorMO;
                }
            },
            {
                //tile: "Collaborator EQ",
                width: "5%",
                data: function (result) {
                    if (result.collaboratorEQ == "0.00")
                        return "";
                    else
                        return result.collaboratorEQ;
                }
            },
            {
                //title: "Opciones",
    ...
    var datatables = {
        ...
        budgetDt: {
            init: function () {
                budgetDatatable = $("#budgets_datatable").DataTable(budgetDtOpt);
                this.initEvents();
            },
            reload: function () {
                        ...
                        Swal.fire({
                            ...
                            preConfirm: () => {
                                return new Promise((resolve) => {
                                    $.ajax({
                                        url: _app.parseUrl(`/oficina-tecnica/presupuestos/eliminar/${id}`),
                                        type: "delete",
    ...
    var forms = {
        load: {
            edit: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/oficina-tecnica/presupuestos/${id}`)
                })
        ...
        submit: {
            add: function (formElement) {
                ...
                $.ajax({
                    url: _app.parseUrl("/oficina-tecnica/presupuestos/crear"),
                    method: "post",
                    data: data,
                    contentType: false,
                    processData: false
            ...
            edit: function (formElement) {
                ...
                $.ajax({
                    url: _app.parseUrl(`/oficina-tecnica/presupuestos/editar/${id}`),
                    method: "put",
                    contentType: false,
                    processData: false,
                    data: data
            ...
            import: {
                data: function (formElement) {
                    ...
                    $.ajax({
                        url: "/oficina-tecnica/presupuestos/importar-datos",
                        type: "POST",
                        contentType: false,
                        processData: false,
                        data: data,
    ...
    var events = {
        excel: function () {
            $("#btn-massive-load").on("click", function () {
                window.location = `/oficina-tecnica/presupuestos/excel-carga-masiva`;
            });
        },
        neto: function () {
            $("#btn-calculate-neto").on("click", function () {
                _app.loader.show();
                $.ajax({
                    url: "/oficina-tecnica/presupuestos/calcular-netos",
                    method: "post",
                    contentType: false,
                    processData: false
                }).always(function () {
        ...
        deleteByFilters: function () {
            $("#btn-delete-by-filters").on("click", function () {
                Swal.fire({
                    title: "¿Está seguro?",
                    text: "Los presupuestos CD filtrados serán eliminados permanentemente",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonText: "Sí, eliminarla",
                    confirmButtonClass: "btn-danger",
                    ...
                    preConfirm: () => {
                        return new Promise((resolve) => {
                            $.ajax({
                                url: _app.parseUrl(`/oficina-tecnica/presupuestos/eliminar-filtro`),
                                data: {
                                ...
                                success: function (result) {
                                    ...
                                    swal.fire({
                                        type: "success",
                                        title: "Completado",
                                        text: "Los presupuestos CD han sido eliminados con éxito",
                                        confirmButtonText: "Excelente"
                                    });
                                },
