var Formulas = function () {
    ...
    var formulasDtOpts = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/equipos/tipo-de-equipo-transporte/listar"),
            dataSrc: ""
        },
        columns: [
            {
    ...
    var activitiesDtOpts = {
        responsive: true,
        ajax: {
            url: _app.parseUrl("/equipos/actividades-tipo-de-equipo-transporte/listar"),
            data: function (d) {
                        ...
                        Swal.fire({
                            ...
                            preConfirm: () => {
                                return new Promise((resolve) => {
                                    $.ajax({
                                        url: _app.parseUrl(`/equipos/tipo-de-equipo-transporte/eliminar/${id}`),
                                        type: "delete",
                    ...
                    function () {
                        ...
                        Swal.fire({
                            ...
                            preConfirm: () => {
                                return new Promise((resolve) => {
                                    $.ajax({
                                        url: _app.parseUrl(`/equipos/actividades-tipo-de-equipo-transporte/eliminar/${id}`),
                                        type: "delete",
                                        success: function (result) {
    ...
    var forms = {
        load: {
            edit: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/equipos/tipo-de-equipo-transporte/${id}`)
                })
            ...
            editActivity: function (id) {
                _app.loader.show();
                $.ajax({
                    url: _app.parseUrl(`/equipos/actividades-tipo-de-equipo-transporte/${id}`),
                    dataSrc: ""
                })
                    .done(function (result) {
                        let formElements = $("#edit_activity_form");
                        formElements.find("[name='Id']").val(result.id);
                        formElements.find("[name='EquipmentMachineryTypeTransportId']").val(result.equipmentMachineryId);
                        formElements.find("[name='Description']").val(result.description);
                        $("#detail_modal").modal("hide");
        ...
        submit: {
            add: function (formElement) {
                ...
                $.ajax({
                    url: _app.parseUrl("/equipos/tipo-de-equipo-transporte/crear"),
                    method: "post",
                    data: data
                })
                    .always(function () {
            ...
            edit: function (formElement) {
                ...
                $.ajax({
                    url: _app.parseUrl(`/equipos/tipo-de-equipo-transporte/editar/${id}`),
                    method: "put",
                    data: data
                })
                    .always(function () {
            ...
            addActivity: function (formElement) {
                $(formElement).find("[name='EquipmentMachineryTypeTransportId']").val(equipmentMachineryId);
                let data = new FormData($(formElement).get(0));
                let $btn = $(formElement).find("button[type='submit']");
                $btn.addLoader();
                $(formElement).find("input, select").prop("disabled", true);
                $.ajax({
                    url: _app.parseUrl("/equipos/actividades-tipo-de-equipo-transporte/crear"),
                    method: "post",
                    data: data,
                    contentType: false,
                    processData: false
            ...
            editActivity: function (formElement) {
                ...
                let id = $(formElement).find("input[name='Id']").val();
                $.ajax({
                    url: _app.parseUrl(`/equipos/actividades-tipo-de-equipo-transporte/editar/${id}`),
                    method: "put",
                    data: data,
                    contentType: false,
                    processData: false
